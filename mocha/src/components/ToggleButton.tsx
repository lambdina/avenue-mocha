...
export const ToggleButton: React.FC<{label:string, callback:any, isDisabled:boolean}> = ({label, callback, isDisabled}) => {
    return (
        <label className="flex relative items-center mb-5 cursor-pointer">
            {isDisabled &&
                <input type="checkbox" value="" id="disabled-toggle" className="sr-only peer" onClick={callback}/> }
            {!isDisabled &&
                <input type="checkbox" value="" id="disabled-toggle" className="sr-only peer" checked onClick={callback}/> }
